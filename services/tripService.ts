...
export async function saveTripToSupabase(
  ...
  try {
    const { notifyTripCreated } = await import('./notificationService');
    await notifyTripCreated(userId, tripData.destination, createdQuestIds.length);
  } catch (err) {
    console.error('Failed to create trip notification:', err);
  }
