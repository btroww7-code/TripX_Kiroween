...
export async function completeQuest(completionData: QuestCompletionData) {
  ...
  try {
    const { notifyQuestCompleted } = await import('./notificationService');
    await notifyQuestCompleted(userId, quest.title, quest.reward_xp, quest.reward_tokens);
  } catch (err) {
    console.error('Failed to create quest completion notification:', err);
  }
