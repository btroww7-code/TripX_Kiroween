...
/**
 * Add a spooky destination quest to user's quests
 * Creates the quest in database if it doesn't exist, then adds to user_quests
 */
export async function addQuestToUser(userId: string, questData: {
  id: string;
  title: string;
  description: string;
  category: string;
  ...
  console.log('[addQuestToUser] Adding quest:', questData.id, 'for user:', userId);

  // First check if quest exists in quests table
  ...
  if (!existingQuest) {
    console.log('[addQuestToUser] Creating new quest in database:', questData.id);
    ...
    const { error: createError } = await supabase
      ...
      .insert({
        ...
        difficulty: difficultyMap[questData.difficulty] || 2,
        reward_tokens: questData.token_reward,
        reward_xp: questData.xp_reward,
        nft_reward: questData.nft_reward,
        quest_type: 'halloween',
        is_active: true,
        is_permanent: false,
        completion_requirements: { requirements: questData.requirements },
        created_at: new Date().toISOString(),
      });

    if (createError) {
      console.error('[addQuestToUser] Error creating quest:', createError);
      // Continue anyway - quest might already exist with different check
    }
  }
  ...
  if (existingUserQuest) {
    console.log('[addQuestToUser] User already has this quest');
    return;
  }

  // Add quest to user_quests
  ...
  if (insertError) {
    console.error('[addQuestToUser] Error adding quest to user:', insertError);
    throw new Error(`Failed to add quest: ${insertError.message}`);
  }

  console.log('[addQuestToUser] Quest added successfully!');
}
