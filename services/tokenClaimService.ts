...
export async function claimTPXRewards(userId: string, walletAddress: string): Promise<{
  ...
  try {
    ...
    if (user) {
      ...
      // Create notification for tokens claimed
      try {
        const { notifyTokensClaimed } = await import('./notificationService');
        await notifyTokensClaimed(userId, totalAmount, hash);
      } catch (err) {
        console.error('Failed to create tokens claimed notification:', err);
      }
