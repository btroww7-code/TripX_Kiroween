...
export async function checkAchievements(userId: string): Promise<Achievement[]> {
  try {
    ...
    if (data && data.length > 0) {
      ...
      // Create notifications for each newly unlocked achievement
      if (achievements && achievements.length > 0) {
        try {
          const { notifyAchievementUnlocked } = await import('./notificationService');
          ...
          console.error('Failed to create achievement notifications:', err);
        }
      }
