...
interface AddNFTToWalletModalProps {
  ...
  questTitle?: string;
  imageUrl?: string;
  refreshOnClose?: boolean; // If true, refresh page when closing
}
          ...
          >
            ...
            {imageUrl && (
              <div className="mb-6 flex justify-center">
                <div className="relative w-48 h-48 rounded-xl overflow-hidden border-2 border-purple-500/50 shadow-lg shadow-purple-500/20">
                  <img 
                    src={imageUrl} 
                    alt={questTitle || 'NFT'} 
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://jjzksgzieuguuolfcekk.supabase.co/storage/v1/object/public/nft-images/achievement-default.png';
                    }}
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                  <div className="absolute bottom-2 left-2 right-2 text-center">
                    <span className="text-xs font-semibold text-white/90 bg-black/40 px-2 py-1 rounded-full">
                      #{tokenId}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              {/* Add to MetaMask / View NFT */}
              <motion.button
                whileHover={{ scale: 1.02, y: -2 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => {
                  // Try to add to wallet, but also provide fallback
                  onAddToWallet();
                }}
              ...
              {/* Note about MetaMask NFT support */}
              <p className="text-xs text-white/40 text-center mt-2">
                💡 If MetaMask doesn't show the NFT, view it on Etherscan below
              </p>
              ...
              <button
                onClick={handleClose}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-pumpkinOrange to-bloodOrange text-white font-semibold hover:opacity-90 transition-all duration-300"
              >
                {refreshOnClose ? 'Done - Close & Refresh' : 'Close'}
              </button>
            </div>
