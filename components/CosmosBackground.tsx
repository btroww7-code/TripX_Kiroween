...
export const CosmosBackground: React.FC<CosmosBackgroundProps> = ({
  ...
  const getStarColor = (): number => {
    if (enableHalloweenMode) {
      // Halloween colors: orange and purple stars
      const rand = Math.random();
      if (rand < 0.4) return 0xff6b35; // bloodOrange
      if (rand < 0.7) return 0xff9500; // pumpkinOrange
      if (rand < 0.85) return 0x1a0a2e; // deepPurple (darker stars)
      if (rand < 0.95) return 0xf0e6ff; // ghostlyWhite
      return 0x39ff14; // toxicGreen (rare)
    }
  ...
  const generateStars = () => {
    ...
    // Reduce animation speeds when reduced motion is preferred
    ...
    for (let i = 0; i < normalStars; i++) {
      ...
      starData.push({
        ...
        color: getStarColor(),
        velocityX: (Math.random() - 0.5) * 0.08 * animationSpeed * motionMultiplier,
        velocityY: (Math.random() - 0.5) * 0.08 * animationSpeed * motionMultiplier,
        velocityZ: (Math.random() - 0.5) * 0.08 * animationSpeed * motionMultiplier,
        twinkleSpeed: (Math.random() * 0.003 + 0.001) * motionMultiplier,
    ...
    for (let i = 0; i < milkyWayStars; i++) {
      ...
      starData.push({
        ...
        velocityX: (Math.random() - 0.5) * 0.03 * animationSpeed * motionMultiplier,
        velocityY: (Math.random() - 0.5) * 0.02 * animationSpeed * motionMultiplier,
        velocityZ: (Math.random() - 0.5) * 0.03 * animationSpeed * motionMultiplier,
        twinkleSpeed: (Math.random() * 0.002 + 0.001) * motionMultiplier,
        twinkleOffset: Math.random() * 1000,
      });
    }
  ...
  const createShootingStar = () => {
    ...
    // Reduce shooting star speed when reduced motion is preferred
    const motionMultiplier = reducedMotion ? 0.2 : 1.0;
    ...
    shootingStarDataRef.current.push({
      ...
      y: radius * Math.sin(phi) * Math.sin(theta),
      z: radius * Math.cos(phi),
      vx: Math.sin(dirPhi) * Math.cos(dirTheta) * 15 * animationSpeed * motionMultiplier,
      vy: Math.sin(dirPhi) * Math.sin(dirTheta) * 15 * animationSpeed * motionMultiplier,
      vz: Math.cos(dirPhi) * 15 * animationSpeed * motionMultiplier,
      life: 1.0,
      size: 4,
    });
  };
          ...
          if (shootingStarsRef.current) {
            ...
            for (let i = shootingStarDataRef.current.length - 1; i >= 0; i--) {
              ...
              if (i3 < ssPositions.length && i3 + 2 < ssPositions.length) {
                ...
                // Halloween mode: orange shooting stars, default: white/blue
                if (enableHalloweenMode) {
                  ssColors[i3] = star.life * 2;     // R: full orange
                  ssColors[i3 + 1] = star.life * 1.2; // G: partial for orange
                  ssColors[i3 + 2] = star.life * 0.5; // B: minimal for orange
                } else {
            ...
            // Reduce frequency when reduced motion is preferred
            const shootingStarFrequency = reducedMotion ? 0.001 : 0.005;
            if (shootingStarDataRef.current.length === 0 && Math.random() < shootingStarFrequency) {
              createShootingStar();
            }
  ...
  // Regenerate stars when reduced motion preference changes
  useEffect(() => {
    if (sceneRef.current && starFieldRef.current) {
      generateStars();
