...
// Helper to calculate level from XP for consistency across all components
// Formula: level = floor(sqrt(totalXP / 100)) + 1
const getLevel = (entry: LeaderboardEntry): number => {
  const xp = getXp(entry);
  return Math.floor(Math.sqrt(xp / 100)) + 1;
};

// Helper to get fresh tokens earned - now uses wallet balance if available
const getTokensEarned = (entry: LeaderboardEntry, walletBalances: Record<string, number>): number => {
  // If user has wallet and we have blockchain balance, use that
  const walletAddress = entry.users?.wallet_address;
  if (walletAddress && walletBalances[walletAddress] !== undefined) {
...
export const Leaderboard: React.FC = () => {
  ...
  const loadLeaderboard = React.useCallback(async () => {
    ...
    try {
      ...
      // Fetch blockchain balances for users with wallets
      if (data && data.length > 0) {
        fetchWalletBalances(data);
      }
    } catch (error) {
  ...
  // Fetch TPX balances from blockchain for all users with wallets
  const fetchWalletBalances = async (entries: LeaderboardEntry[]) => {
    const walletsToFetch = entries
      .filter(e => e.users?.wallet_address)
      .map(e => e.users!.wallet_address!);
    
    if (walletsToFetch.length === 0) return;
    
    setLoadingBalances(true);
    console.log('[Leaderboard] Fetching blockchain balances for', walletsToFetch.length, 'wallets');
    
    const balances: Record<string, number> = {};
    
    // Fetch balances in parallel (max 5 at a time to avoid rate limits)
    const batchSize = 5;
    for (let i = 0; i < walletsToFetch.length; i += batchSize) {
      const batch = walletsToFetch.slice(i, i + batchSize);
      const results = await Promise.all(
        batch.map(async (wallet) => {
          try {
            const balance = await getTPXBalance(wallet);
            return { wallet, balance };
          } catch (error) {
            console.warn('[Leaderboard] Failed to fetch balance for', wallet);
    ...
    console.log('[Leaderboard] Fetched balances:', balances);
                        ...
                        <td className="px-4 lg:px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <span className="font-bold text-amber-400 text-sm lg:text-base">
                              {loadingBalances && entry.users?.wallet_address ? '...' : getTokensEarned(entry, walletBalances).toLocaleString()}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 lg:px-6 py-4 text-right">
