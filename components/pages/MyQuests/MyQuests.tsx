...
import { QuestPhotoVerificationModal } from '../../QuestPhotoVerificationModal';
...
export const MyQuests: React.FC<MyQuestsProps> = ({ onNavigate }) => {
  ...
  const [achievementMintResult, setAchievementMintResult] = useState<{tokenId: number, txHash: string, contractAddress: string, questTitle: string, imageUrl: string} | null>(null);
  ...
  // New unified verification modal state
  const [verificationQuest, setVerificationQuest] = useState<any | null>(null);
  ...
  const handleMintNFT = async (userQuest: any) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Get quest image URL for NFT display
        const questImageUrl = quest.image_url || getQuestImageUrl(quest.title, quest.location || quest.location_name || '');
  ...
  return (
    <div className="max-w-7xl mx-auto">
              ...
              <div className="flex gap-2">
                {!isCompleted && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => {
                      // Use new unified verification modal (TPX + NFT claimed together)
                      if (address) {
                        setVerificationQuest(userQuest);
                      } else {
      ...
      {/* OLD Quest Detail Modal - DISABLED, replaced by QuestPhotoVerificationModal */}
      {/* The old modal with separate TPX claim and NFT mint has been replaced
          by the new unified QuestPhotoVerificationModal which claims both simultaneously */}
      {false && selectedQuest && (
        <AnimatePresence>
          <motion.div
            initial={{ opacity: 0 }}
      ...
      {achievementMintResult && (
        <AddNFTToWalletModal
          ...
          imageUrl={achievementMintResult.imageUrl}
        />
      )}
      ...
      {showQuestCelebration && (
        <HalloweenCelebration
          type="quest"
          onComplete={() => setShowQuestCelebration(false)}
        />
      )}

      {/* New Unified Quest Verification Modal (TPX + NFT claimed together) */}
      {verificationQuest && user && address && (
        <QuestPhotoVerificationModal
          quest={{
            id: verificationQuest.quests.id,
            title: verificationQuest.quests.title,
            description: verificationQuest.quests.description,
            location: verificationQuest.quests.location,
            reward_tokens: verificationQuest.quests.reward_tokens || 0,
            reward_xp: verificationQuest.quests.reward_xp || 0,
            nft_reward: verificationQuest.quests.nft_reward || false,
            image_url: verificationQuest.quests.image_url,
          }}
          userId={user.id}
          walletAddress={address}
