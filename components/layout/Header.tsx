...
import { Coins, TrendingUp, Bell, Award, X, Mail, LogOut, Check, Trash2 } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  const { notifications, unreadCount, markAsRead, markAllAsRead, deleteNotif } = useNotifications(user?.id);
              ...
              >
                ...
                {unreadCount > 0 && (
                  <motion.div
                    ...
                    className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-gradient-to-br from-red-500 to-red-600 rounded-full shadow-lg shadow-red-400/50 flex items-center justify-center"
                  >
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-96 rounded-2xl backdrop-blur-xl border border-white/[0.08] shadow-2xl z-[100] overflow-hidden"
                    style={{
                  ...
                  >
                    <div className="p-4 border-b border-white/[0.05] flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-white">
                        Powiadomienia
                        {unreadCount > 0 && (
                          <span className="ml-2 text-sm text-cyan-400">({unreadCount})</span>
                        )}
                      </h3>
                      ...
                      {notifications.length > 0 ? (
                        notifications.map((notification, idx) => (
                          <motion.div
                            ...
                            className={`p-4 border-b border-white/[0.03] hover:bg-white/[0.05] transition-colors cursor-pointer group relative ${
                            ...
                            onClick={() => {
                              ...
                              if (notification.action_url && onNavigate) {
                                const page = notification.action_url.replace('/', '');
                                onNavigate(page);
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                                notification.type === 'success' || notification.type === 'quest' || notification.type === 'achievement' ? 'bg-green-400' :
                                notification.type === 'warning' ? 'bg-yellow-400' :
                                notification.type === 'error' ? 'bg-red-400' :
                                notification.type === 'transaction' || notification.type === 'token' ? 'bg-amber-400' :
                                notification.type === 'nft' ? 'bg-purple-400' :
                                notification.type === 'trip' ? 'bg-blue-400' :
                                'bg-cyan-400'
                              }`} />
